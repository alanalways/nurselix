...
export async function GET(req: NextRequest) {
  ...
  const now = new Date();
  // Hard-coded mail kill switch — keep in sync with lib/mail.ts MAIL_DISABLED.
  // Downgrade logic below always runs regardless.
  ...
  const warnDays = [1, 3];
  let warnSent = 0;
  if (!MAIL_DISABLED) for (const days of warnDays) {
