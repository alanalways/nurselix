...
【主題內容分配（共 25 題）】
1. acute illness management — MI, stroke, DKA（急性疾病處置）：3 題
2. hemodynamic monitoring / shock management（血流動力學監測與休克處置）：3 題
3. fluid and electrolyte imbalances（水電解質不平衡）：3 題
4. respiratory emergencies — ARDS, PE, pneumothorax（呼吸緊急狀況）：3 題
5. cardiac monitoring / dysrhythmia recognition（心臟監測與心律不整辨識）：3 題
6. wound care / infection management（傷口護理與感染處置）：2 題
7. neurological emergencies — ICP, seizure, stroke（神經系統緊急狀況）：3 題
8. endocrine crises — DKA, HHS, thyroid storm（內分泌危象）：2 題
9. medical emergencies / rapid response / code（醫療緊急應變）：3 題
...
7. 生命徵象、實驗室數值、臨床判斷標準必須正確（如正常 SpO₂、血鉀、pH 值範圍）。
...
3. id 從 rn2026_pa_XXXX 連續編號（XXXX = start_index 補零至 4 位）。
...
7. 所有選項 id 格式：rn2026_pa_XXXX_a 到 rn2026_pa_XXXX_f。
