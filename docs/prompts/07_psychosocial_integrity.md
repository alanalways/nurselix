...
【主題內容分配（共 25 題）】
1. therapeutic communication techniques（治療性溝通技巧）：3 題
2. mental health disorders — depression, anxiety, schizophrenia, bipolar（精神疾病護理）：3 題
3. crisis intervention and de-escalation（危機介入與情緒降溫）：3 題
4. grief, loss, and end-of-life care（悲傷、失落與臨終照護）：3 題
5. abuse, neglect, and mandatory reporting（虐待、疏忽與強制通報）：3 題
6. substance use disorders and withdrawal（物質使用障礙與戒斷）：2 題
7. coping mechanisms and stress management（因應機制與壓力管理）：3 題
8. cultural sensitivity and spiritual care（文化敏感度與靈性照護）：2 題
9. behavioral health safety — suicide risk, elopement, seclusion（行為健康安全）：3 題
...
7. 治療性溝通題型：避免 blocking responses（給建議、轉移話題、過度安慰、使用陳腔濫調）。
8. SATA 題的正確選項間不可有明顯邏輯重疊。
9. 自殺風險評估、強制住院、保密限制等法律與倫理細節必須正確。
...
3. id 從 rn2026_psi_XXXX 連續編號（XXXX = start_index 補零至 4 位）。
...
7. 所有選項 id 格式：rn2026_psi_XXXX_a 到 rn2026_psi_XXXX_f。
